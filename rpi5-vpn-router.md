...
3. Töltsd le a **Factory (EXT4)** image-et (`.img.gz`)
...
### Tailscale automatikus exit node boot után

Hozz létre egy script-et:

```bash
cat << 'EOF' > /etc/tailscale-exitnode.sh
#!/bin/sh
# Vár amíg a Tailscale csatlakozik, majd beállítja az exit node-ot
sleep 30
tailscale set --exit-node=<HOSTNAME>
EOF
chmod +x /etc/tailscale-exitnode.sh
```

Add hozzá az `/etc/rc.local`-hoz (`exit 0` elé):

```bash
/etc/tailscale-exitnode.sh &
```
